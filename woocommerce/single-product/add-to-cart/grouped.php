...
<form class="cart grouped_form" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table">
		<tbody>
			...
			foreach ($grouped_products as $grouped_product_child) {
				...
				echo '<a href="' . esc_url($grouped_product_child->get_permalink()) . '">';

				if (has_post_thumbnail($grouped_product_child->get_id())) {
					// echo $grouped_product_child->get_image('full');
					echo get_the_post_thumbnail($grouped_product_child->get_id(), 'shop_thumbnail', array('alt' => $grouped_product_child->get_name()));
				} else {
					echo '<img src="' . esc_url(get_template_directory_uri() . '/assets/images/100x150.png') . '" alt="Default Image">';
				}
					...
					switch ($column_id) {
						case 'quantity':
							...
							if (! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock()) {
								...
								echo '<label for="' . esc_attr('quantity-' . $grouped_product_child->get_id()) . '" class="screen-reader-text">' . esc_html__('Buy one of this item', 'nebon') . '</label>';
							} else {
							...
							break;
						case 'label_price':
							$value  = '<div class="product-label-price-sold">';
							$value .= '<label for="product-' . esc_attr($grouped_product_child->get_id()) . '">';
							$value .= $grouped_product_child->is_visible()
								? '<a href="' . esc_url(apply_filters('woocommerce_grouped_product_list_link', $grouped_product_child->get_permalink(), $grouped_product_child->get_id())) . '">' . $grouped_product_child->get_name() . '</a>'
							...
							$stock_quantity = $grouped_product_child->get_stock_quantity();
							$sold_quantity = get_post_meta($grouped_product_child->get_id(), 'total_sales', true);
							...
							$value .= '<div class="product-progress-bar">';
							$value .= '<div class="progress">';
							$value .= '<div class="progress-fill" style="width:' . esc_attr($percentage_sold) . '%;"></div>';
							$value .= '</div>';
							$value .= '<div class="progress-text">';
							$value .= '<div class="progress-sold">';
							$value .= '<span class="label">' . esc_html__('Sold:', 'nebon') . '</span>';
							$value .= '<span class="quantity">' . esc_html($sold_quantity) . '</span>';
							$value .= '</div>';

							$value .= '<div class="progress-available">';
							$value .= '<span class="label">' . esc_html__('Available:', 'nebon') . '</span>';
							$value .= '<span class="quantity">' . esc_html($stock_quantity) . '</span>';
							$value .= '</div>';
	...
	<?php if ($quantites_required && $show_add_to_cart_button) : ?>
		...
		<div class="grouped-product-actions">
			...
			<?php t888f_get_template('woocommerce/single-product-structure/user-action', '', array(), true); ?>
