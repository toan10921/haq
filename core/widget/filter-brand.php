...
if (!class_exists('tech888f_FilterBrand_Widget')) {
    class tech888f_FilterBrand_Widget extends WP_Widget
    {
        protected $default = array();
        protected $widget_name = 'filter-brand';
        ...
        static function _add_widget()
        {
            if (function_exists('tech888f_reg_widget'))
                tech888f_reg_widget('tech888f_FilterBrand_Widget');
        }

        function __construct()
        {
            parent::__construct(
                'tech888f_filter_brand_widget',
                __('7up Custom Brand Filter Widget', 'nebon'),
                array('description' => __('A widget to filter products by brand', 'nebon'))
            );

            $this->default = array(
                'title' => esc_html__('Filter by Brand', 'nebon'),
            );
        ...
        function widget($args, $instance)
        {
            ...
            $title = !empty($instance['title']) ? $instance['title'] : __('Filter by Brand', 'nebon');

            tech888f_get_template_widget('filter-brand', '', array(
                'title' => $title,
                'args'  => $args,
...
<?php
    ...
    tech888f_FilterBrand_Widget::_init();
}
