...
$raw_placeholder = isset($placeholder) ? trim((string) $placeholder) : '';
$legacy_placeholders = ['What are you searching for ?', 'What are you searching for?', 'Search ...'];
$placeholder_text = ($raw_placeholder === '' || in_array($raw_placeholder, $legacy_placeholders, true))
    ? 'Tìm kiếm sản phẩm…'
    : $raw_placeholder;
...
$form_action = ($selected_post_type === 'product' && function_exists('wc_get_page_permalink'))
...
<div class="t888-search-form-inline-wrap style2">
    <form class="t888-search-form-inline search-form <?php echo esc_attr($use_ajax === 'yes' ? 'search-ajax' : ''); ?>" action="<?php echo esc_url($form_action); ?>" method="get" role="search">
        <label class="d-none" for="t888-search-inline-input"><?php echo esc_html__('Search', 'nebon'); ?></label>
        <input id="t888-search-inline-input" name="<?php echo esc_attr($selected_post_type === 'product' ? 'product_search' : 's'); ?>" type="search"
        ...
        <?php if ($show_cats === 'yes' && !empty($categories)) : ?>
            <div class="custom-dropdown custom-dropdown-categories">
                <div class="custom-dropdown-toggle-search position-relative">
                    <?php echo esc_html('Tất cả danh mục'); ?>
                </div>
            ...
            <select name="<?php echo esc_attr($selected_post_type === 'product' ? 'product_cat' : 'category_name'); ?>" class="form-select d-none">
