...
class Customizer_Repeater extends WP_Customize_Control
{
	...
	public $customizer_repeater_image_only_control = false;
	public $customizer_repeater_icon_control = false;
	public $customizer_repeater_icon_only_control = false;
	...
	public $customizer_repeater_upload_file_control = false;
	public $customizer_repeater_upload_file_only_control = false;
	public $type = 'customizer-repeater';
	...
	public function __construct($manager, $id, $args = array())
	{
		...
		$this->add_field_label = esc_html__('Add new field', 'nebon');
		if (! empty($args['add_field_label'])) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__('Customizer Repeater', 'nebon');
		...
		if (! empty($args['customizer_repeater_image_only_control'])) {
			$this->customizer_repeater_image_only_control = $args['customizer_repeater_image_only_control'];
		}
		...
		if (! empty($args['customizer_repeater_icon_only_control'])) {
			$this->customizer_repeater_icon_only_control = $args['customizer_repeater_icon_only_control'];
		}
		...
		if (! empty($args['customizer_repeater_upload_file_control'])) {
			$this->customizer_repeater_upload_file_control = $args['customizer_repeater_upload_file_control'];
		}
		if (! empty($args['customizer_repeater_upload_file_only_control'])) {
			$this->customizer_repeater_upload_file_only_control = $args['customizer_repeater_upload_file_only_control'];
		}
		if (! empty($id)) {
			$this->id = $id;
		}

		if (file_exists(get_template_directory() . '/customizer-repeater/inc/icons2.php')) {
			$this->customizer_icon_container =  'customizer-repeater/inc/icons2';
		}
	...
	public function enqueue()
	{
		...
		wp_enqueue_style('line-awesome', get_template_directory_uri() . '/customizer-repeater/css/line-awesome.min.css', array(), CUSTOMIZER_REPEATER_VERSION);
		...
		wp_enqueue_script('customizer-repeater-lineawesome-iconpicker', get_template_directory_uri() . '/customizer-repeater/js/lineawesome-iconpicker.min.js', array('jquery'), CUSTOMIZER_REPEATER_VERSION, true);

		wp_enqueue_style('customizer-repeater-lineawesome-iconpicker-script', get_template_directory_uri() . '/customizer-repeater/css/lineawesome-iconpicker.min.css', array(), CUSTOMIZER_REPEATER_VERSION);
	}
		...
		if (!empty($array)) {
			foreach ($array as $icon) { ?>
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						<?php
						$choice = $file_url = $file_only_url = $image_url = $image_only_url = $icon_value = $icon_only_value = $title = $subtitle = $text = $text2  = $link2 = $link = $shortcode = $repeater = $color = $color2 = '';
						if (!empty($icon->id)) {
							$id = $icon->id;
						}
						...
						if (!empty($icon->image_only_url)) {
							$image_only_url = $icon->image_only_url;
						}
						if (!empty($icon->icon_only_value)) {
							$icon_only_value = $icon->icon_only_value;
						}
						...
						if (!empty($icon->file_url)) {
							$file_url = $icon->file_url;
						}
						if (!empty($icon->file_only_url)) {
							$file_only_url = $icon->file_only_url;
						}

						if ($this->customizer_repeater_image_only_control == true) {
							$this->image_only_control($image_only_url, 'customizer_repeater_image');
						}

						if ($this->customizer_repeater_image_control == true && $this->customizer_repeater_icon_control == true) {
							$this->icon_type_choice($choice);
						}
						if ($this->customizer_repeater_upload_file_only_control == true) {
							$this->upload_file_only_control($file_only_url, 'customizer_repeater_file');
						}
						if ($this->customizer_repeater_upload_file_control == true && $this->customizer_repeater_icon_control == true) {
							$this->icon_type_choice($choice);
						}
						if ($this->customizer_repeater_image_control == true) {
							$this->image_control($image_url, $choice);
						}
						if ($this->customizer_repeater_upload_file_control == true) {
							$this->upload_file_control($file_url, $choice);
						}
						if ($this->customizer_repeater_icon_only_control == true) {
							$this->icon_only_picker_control($icon_only_value, 'customizer_repeater_icon');
						}
						...
						if ($this->customizer_repeater_color_control == true) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Color', 'nebon'), $this->id, 'customizer_repeater_color_control'),
						...
						if ($this->customizer_repeater_color2_control == true) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Color', 'nebon'), $this->id, 'customizer_repeater_color2_control'),
						...
						if ($this->customizer_repeater_title_control == true) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Title', 'nebon'), $this->id, 'customizer_repeater_title_control'),
						...
						if ($this->customizer_repeater_subtitle_control == true) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Subtitle', 'nebon'), $this->id, 'customizer_repeater_subtitle_control'),
						...
						if ($this->customizer_repeater_text_control == true) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Text', 'nebon'), $this->id, 'customizer_repeater_text_control'),
						...
						if ($this->customizer_repeater_link_control) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Link', 'nebon'), $this->id, 'customizer_repeater_link_control'),
						...
						if ($this->customizer_repeater_text2_control == true) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Text', 'nebon'), $this->id, 'customizer_repeater_text2_control'),
						...
						if ($this->customizer_repeater_link2_control) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Link', 'nebon'), $this->id, 'customizer_repeater_link2_control'),
						...
						if ($this->customizer_repeater_shortcode_control == true) {
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__('Shortcode', 'nebon'), $this->id, 'customizer_repeater_shortcode_control'),
						...
						<button type="button" class="social-repeater-general-control-remove-field" <?php if ($it == 0) {
							...
							<?php esc_html_e('Delete field', 'nebon'); ?>
						</button>
			...
			<div class="customizer-repeater-general-control-repeater-container">
				...
				<div class="customizer-repeater-box-content-hidden">
					<?php
					if ($this->customizer_repeater_image_only_control == true) {
						$this->image_only_control('', 'customizer_repeater_image');
					}
					if ($this->customizer_repeater_upload_file_only_control == true) {
						$this->upload_file_only_control('', 'customizer_repeater_file');
					}
					if (
						$this->customizer_repeater_icon_control
						|| $this->customizer_repeater_image_control
						|| $this->customizer_repeater_upload_file_control
					) {
					...
					if ($this->customizer_repeater_upload_file_control == true) {
						$this->upload_file_control();
					}
					if ($this->customizer_repeater_icon_only_control == true) {
						$this->icon_only_picker_control('', 'customizer_repeater_icon');
					}
					if ($this->customizer_repeater_icon_control == true) {
	...
	private function icon_picker_control($value = '', $show = '')
	{
		...
		<div class="social-repeater-general-control-icon" <?php if ($show === 'customizer_repeater_image' || $show === 'customizer_repeater_none') {
			...
			<span class="customize-control-title">
				<?php esc_html_e('Icon', 'nebon'); ?>
			</span>
			<span class="description customize-control-description">
				<?php
				echo sprintf(
					esc_html__('Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'nebon'),
					sprintf('<span >%s</span>', esc_html__('Offical Website: icons8.com/line-awesome', 'nebon'))
				); ?>
			</span>
	...
	private function icon_only_picker_control($value = '', $show = '')
	{
	?>
		<div class="social-repeater-general-control-icon" <?php if ($show === 'customizer_repeater_image' || $show === 'customizer_repeater_none') {
																echo 'style="display:none;"';
			...
			<div class="input-group icp-container">
				<input data-placement="bottomRight" class="icp icp-auto icp-only" value="<?php if (!empty($value)) {
																								echo esc_attr($value);
																							} ?>" type="text">
				<span class="input-group-addon">
					<i class="<?php echo esc_attr($value); ?>"></i>
	...
	{ ?>
		<div class="customizer-repeater-image-control" <?php if ($show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty($show)) {
			...
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e('Upload Image', 'nebon'); ?>" />
		</div>
	<?php
	}

	private function image_only_control($value = '', $show = '')
	{ ?>
		<div class="customizer-repeater-image-only-control" <?php if ($show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty($show)) {
																echo 'style="display:none;"';
															} ?>>
			<span class="customize-control-title">
				<?php esc_html_e('Image', 'nebon') ?>
			</span>
			<input type="text" class="widefat custom-media-url_image_only" value="<?php echo esc_attr($value); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e('Upload Image', 'nebon'); ?>" />
		</div>
	<?php
	}
	private function upload_file_control($value = '', $show = '')
	{ ?>
		<div class="customizer-repeater-file-control" <?php if ($show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty($show)) {
															echo 'style="display:none;"';
														} ?>>
			<span class="customize-control-title">
				<?php esc_html_e('File', 'nebon') ?>
			</span>
			<input type="text" class="widefat custom-media-url-file" value="<?php echo esc_attr($value); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e('Upload File', 'nebon'); ?>" />
		</div>
	<?php
	}
	private function upload_file_only_control($value = '', $show = '')
	{ ?>
		<div class="customizer-repeater-file-only-control" <?php if ($show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty($show)) {
																echo 'style="display:none;"';
															} ?>>
			<span class="customize-control-title">
				<?php esc_html_e('File', 'nebon') ?>
			</span>
			<input type="text" class="widefat custom-media-url-file-only" value="<?php echo esc_attr($value); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e('Upload File', 'nebon'); ?>" />
		</div>
	<?php
	}
	...
	{ ?>
		<span class="customize-control-title">
			<?php esc_html_e('Image type', 'nebon'); ?>
		</span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected($value, 'customizer_repeater_icon'); ?>><?php esc_html_e('Icon', 'nebon'); ?></option>
			<option value="customizer_repeater_image" <?php selected($value, 'customizer_repeater_image'); ?>><?php esc_html_e('Image', 'nebon'); ?></option>
			<option value="customizer_repeater_file" <?php selected($value, 'customizer_repeater_file'); ?>><?php esc_html_e('File', 'nebon'); ?></option>
			<option value="customizer_repeater_none" <?php selected($value, 'customizer_repeater_none'); ?>><?php esc_html_e('None', 'nebon'); ?></option>
	...
	private function repeater_control($value = '')
	{
		...
		$show_del        = 0; ?>
		<span class="customize-control-title"><?php esc_html_e('Social icons', 'nebon'); ?></span>
		<?php
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link"
						placeholder="<?php esc_attr_e('Link', 'nebon'); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e('Remove Icon', 'nebon'); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value="" />
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e('Add Icon', 'nebon'); ?></button>
		<?php
		} else { ?>
			<div class="customizer-repeater-social-repeater">
				<?php
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
							style="<?php if ($show_del == 1) {
										echo "display:none";
									} ?>"><?php esc_html_e('Remove Icon', 'nebon'); ?></button>
					</div>
				<?php
				} ?>
