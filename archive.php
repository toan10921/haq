...
class ArchivePage
{
    ...
    private function __construct()
    {
        ...
        $this->sidebar = get_theme_mod('sidebar_blog', 'blog-sidebar');
   ...
   public function render()
{
    ...
    ?>
    <section id="main-content" class="main-page-default blog-page <?php echo esc_attr( t888f_get_main_class_sidebar($layout_pos_for_class) ); ?>">
        <div class="container">
            <div class="row">
                <?php
                if ($has_sidebar && $this->sidebar_pos === 'left') {
                    t888f_get_side_bar('left', $sidebar_id, 'left');
                }
                ?>

                <div class="<?php echo esc_attr( t888f_get_sibling_class_sidebar($layout_pos_for_class) ); ?>">
                    <?php
                    $display_style = $this->args['display_style'] ?? 'default';
                    $is_masonry    = ($display_style === 'masory');
                    $gutter_class  = $is_masonry ? 'gutter-20' : '';
                    $masonry_class = $is_masonry ? 'masonry-grid' : '';
                    ?>
                    <div class="blog-wrap blog-<?php echo esc_attr($this->style); ?> <?php echo esc_attr($masonry_class); ?> <?php echo esc_attr($gutter_class); ?>">

                        <?php if (have_posts()) : ?>
                            <?php $columns = ($this->style === 'grid') ? 'grid-columns-' . esc_attr($this->blog_grid_columns) : ''; ?>
                    ...
                    <?php tech888f_paging_nav(false, 'style2', true); ?>
                </div>

                <?php
                if ($has_sidebar && $this->sidebar_pos === 'right') {
